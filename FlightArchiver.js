function main() {
    ...
    // Call the function to remove duplicates based on CHFLTN, CHPTOL, and CHSTOL
  ...
  function uploadDataToSheet(data, sheetId) {
    ...
    // Restore the first row formatting
    firstRowRange.setFontWeights(firstRowFontWeights);
    firstRowRange.setFontStyles(firstRowFontStyles);
  
    // Resize columns only if new width is greater than current width
  ...
  function removeDuplicatesBasedOnMultipleColumns(sheetId) {
    const sheet = SpreadsheetApp.openById(sheetId).getSheetByName("Sheet1");
    ...
    // Loop through the rows to identify unique rows based on CHFLTN, CHPTOL, and CHSTOL
    for (let i = 1; i < data.length; i++) {
      ...
      Logger.log(`Row ${i}: CHFLTN = ${chfltnValue}, CHPTOL = ${chptolValue}, CHSTOL = ${chstolValue}`);
  
      if (!seenValues.has(combinedValue)) {
        uniqueData.push(data[i]);
        seenValues.add(combinedValue);
      } else {
        Logger.log(`Duplicate found at Row ${i}: ${combinedValue}`);
      }
    }
  
    Logger.log(`Total unique rows: ${uniqueData.length}`);
  
    // Clear the sheet
    sheet.clear();
  
    // Set the unique rows back to the sheet
    const newRange = sheet.getRange(1, 1, uniqueData.length, uniqueData[0].length);
    newRange.setValues(uniqueData);
  
    // Adjust the page to remove empty rows
    sheet.getRange(uniqueData.length + 1, 1, sheet.getMaxRows() - uniqueData.length, sheet.getMaxColumns()).clear();
    sheet.deleteRows(uniqueData.length + 1, sheet.getMaxRows() - uniqueData.length);
  
    Logger.log(`Removed ${data.length - uniqueData.length} duplicate rows based on CHFLTN, CHPTOL, and CHSTOL and adjusted the sheet.`);
  }
